...
namespace UnityEditor.Herl
{
  internal class SimpleLitShaderGUI : BaseShaderGUI
  {
    ...
    private MaterialProperty normalNoiseGridIntensity;
    private MaterialProperty normalNoiseGridWidth;
    private MaterialProperty normalNoiseGridResolution;
    ...
    public override void FindProperties(MaterialProperty[] properties)
    {
      ...
      vertexColor = FindProperty("_IntensityVC", properties);

      normalNoiseGridIntensity = FindProperty("_NormalNoiseGridIntensity", properties);
      normalNoiseGridWidth = FindProperty("_NormalNoiseGridWidth", properties);
      normalNoiseGridResolution = FindProperty("_NormalNoiseGridResolution", properties);
    }
  ...
  public override void ShaderPropertiesGUI(Material material)
    {
      EditorGUI.BeginChangeCheck();
      {
        ...
        materialEditor.ShaderProperty(vertexColor, "Vertex Color");

        materialEditor.ShaderProperty(normalNoiseGridIntensity, "Normal Noise Grid Intensity");
        materialEditor.ShaderProperty(normalNoiseGridWidth, "Normal Noise Grid Width");
        materialEditor.ShaderProperty(normalNoiseGridResolution, "Normal Noise Grid Resolution");
      }
    ...
    private void SetMaterialKeywords(Material material)
    {
      ...
      float vertexColorIntensity = material.GetFloat("_IntensityVC");
      if (vertexColorIntensity <= 0f)
      {
        CoreUtils.SetKeyword(material, "_VERTEXCOLOR_LERP", false);
        CoreUtils.SetKeyword(material, "_VERTEXCOLOR", false);
      }
      else if (vertexColorIntensity > 0f && vertexColorIntensity <= 1f)
      {
        CoreUtils.SetKeyword(material, "_VERTEXCOLOR_LERP", true);
        CoreUtils.SetKeyword(material, "_VERTEXCOLOR", false);
      }
      else
      {
        CoreUtils.SetKeyword(material, "_VERTEXCOLOR_LERP", false);
        CoreUtils.SetKeyword(material, "_VERTEXCOLOR", true);
      }

      float normalNoiseGridIntensity = material.GetFloat("_NormalNoiseGridIntensity");
      float normalNoiseGridWidth = material.GetFloat("_NormalNoiseGridWidth");
      float normalNoiseGridResolution = material.GetFloat("_NormalNoiseGridResolution");
      bool normalNoiseGrid =
        (normalNoiseGridIntensity > 0f) &&
        (normalNoiseGridWidth > 0f) &&
        (normalNoiseGridResolution > 0f);
      CoreUtils.SetKeyword(material, "_NORMALNOISEGRID", normalNoiseGrid);
    }
